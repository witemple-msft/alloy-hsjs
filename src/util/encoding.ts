...
import { Encoder, useJsScalar } from "../core/scalar.jsx";
...
const ENCODE_SOURCES: {
  [k in "ModelProperty" | "Scalar"]: (t: ModelProperty | Scalar) => Type;
} = {
 ...
 */
export function useEncodingChain(
  ...
  for (const [kind, select] of Object.entries(ENCODE_SOURCES)) {
    while (encodeSource.kind === kind) {
      ...
      const encoding = getEncode(program, encodeSource);
      ...
      if (s.kind !== "Scalar") {
        ...
        const sourceJsScalar = useJsScalar(s, encodeSource);
